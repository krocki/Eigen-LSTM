...
int main() {
	...
	size_t test_every_seconds = 60;

	double learning_rate = 1e-1;
	size_t epochs = 1000;

	Eigen::MatrixXi _data = rawread("alice29.txt");
	std::string out_filename = "test";

	//select 'train_percent' % of data as training data
	size_t train_percent = 95;
	size_t percent_size = _data.size() / 100;

	Eigen::MatrixXi data = _data.block(0, 0, train_percent * percent_size, 1); // first x% * percent_size
	Eigen::MatrixXi test = _data.block(train_percent * percent_size, 0, _data.size() - train_percent * percent_size, 1);
	...
	if (data.size() + test.size() != _data.size())
		std::cout << data.size() + test.size() << " != " << _data.size() << " !!!" << std::endl;

	//test = data;
	// read text
	// Eigen::MatrixXi data = rawread("alice29_train.txt");
	// Eigen::MatrixXi test = rawread("alice29_test.txt");
	...
	//set biases of f gates to 1 (http://jmlr.org/proceedings/papers/v37/jozefowicz15.pdf)
	p.b.block(2 * N, 0, N, 1) = Eigen::MatrixXd::Ones(N, 1);
	...
	for (size_t e = 0; e < epochs; e++) {
		...
		for (size_t i = S; i < length; i++) {
			...
			double time_since_start = main_timer.end();

			if (test_time > test_every_seconds) {

				double test_loss = 0;
				double train_error = epoch_loss * double(length) / double(i);
				...
				std::cout << "Train error: " << train_error << ", Test error: " << test_error / double((test_length - 1)) << std::endl;

				results_size++;

				Eigen::MatrixXd current = Eigen::MatrixXd::Zero(1, 7);

				current(0, 0) = (float)(results_size);
				current(0, 1) = e + (float)(i + 1) / (float)length;
				current(0, 2) = iterations;
				current(0, 3) = test_time * (results_size - 1);
				current(0, 4) = train_error;
				current(0, 5) = test_error / double((test_length - 1));
				current(0, 6) = gflops_per_sec;

				std::cout << current << std::endl << std::endl;

				Eigen::MatrixXd new_results = Eigen::MatrixXd::Zero(results.rows() + 1, 7);

				if (results_size > 1)
					new_results.block(0, 0, results.rows(), 7) = results;

				new_results.block(results_size - 1, 0, 1, 7) = current;

				results = Eigen::MatrixXd::Zero(results.rows() + 1, 7);
				results = new_results;

				save_matrix_to_file(results, out_filename);
			...
			if (((i + 1) % 100) == 0) {
				...
				eta_sec = eta_sec % 60;
				gflops_per_sec = (100.0f * flops_per_iteration / powf(2, 30)) / flops_time;
				...
				std::cout << std::setw(15) << "[Epoch " << e + 1 << "/" << epochs << "]" << std::fixed <<
						  ...
						  std::setw(9) << std::setprecision(2) << gflops_per_sec <<
			...
			for (size_t b = 0; b < B; b++) {
				...
				if (positions[b] == S) {
					...
					randnblock(lstm.h[0].col(b), 0, 0.1);
					randnblock(lstm.c[0].col(b), 0, 0.1);
				}
			...
			// loss:
			for (size_t t = S - 1; t < S; t++) { // compute activations for sequence
			...
			epoch_loss += loss / (B * length); // loss/char
